...
export class IngestaComponent implements OnInit {
  ...
  ejecutarIngesta(): void {
    if (
      ...
      this.mostrarError(
        'Por favor, seleccione todas las opciones antes de ejecutar'
      );
      return;
    }

    // Aquí iría la lógica de ejecución real
    const fuenteLabel = this.obtenerEtiqueta(
      this.fuenteSeleccionada,
      this.fuentesDatos
    );
    const claseLabel = this.obtenerEtiqueta(
      this.claseSeleccionada,
      this.clases
    );
    const versionLabel = this.obtenerEtiqueta(
      this.versionSeleccionada,
      this.versiones
    );
    ...
    this.optionsService.resetDatabase().subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.toastService.showError('Error al reiniciar la base de datos');
          // this.mostrarMensaje = true;
          setTimeout(() => (this.mostrarMensaje = false), 3000);
        }
